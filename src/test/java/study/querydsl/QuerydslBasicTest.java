...
public class QuerydslBasicTest {
    ...
    void join() {
        List<Member> result = queryFactory
                .selectFrom(member)
                .join(member.team, team)
                //.leftJoin(), rightJoin() 도 가능하다.
                .where(team.name.eq("teamA"))
                .fetch();

        assertThat(result)
                .extracting("usertname")
                .containsExactly("member1", "member2");
    }

    // 연관관계가 없이도 조인 할 수 있다. 세타 조인
    @Test
    void theta_join() {
    ...
    void selectSubQuery() {
        ...
        List<Tuple> result = queryFactory
                .select(member.username,
                        //JPAExpressions static import를 할 수 있다.
                        select(memberSub.age.avg())
                                .from(memberSub))
                .from(member)
                .fetch();
    ...
    @Test
    void findDtoByField() {
        //Projections.fields get set이 없어도 됨.
        List<MemberDto> result = queryFactory
    ...
    private List<Member> searchMember1(String usernameCond, Integer ageCond) {
        //파라미터의 값이 null이냐 아니냐에 따라서 쿼리가 동적으로 바뀐다.
        BooleanBuilder builder = new BooleanBuilder(/*초기 값을 넣을 수 있다. member.username.eq(usernameCond) */);
    ...
    //이 방법은 실무에서 제일 많이 사용하고 좋아하는 방식입니다.
    @Test
    ...
    // 이 방법을 사용하면 좋은 장점이 된다. 재활용이 가능한 메서드가 됨. (allEq 참고)
